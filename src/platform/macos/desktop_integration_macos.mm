...
namespace lumin {
    ...
    void ConfigureWallpaperWindow(void* windowHandle, const MonitorInfo& monitor) {
        NSWindow* engineWindow = (__bridge NSWindow*)windowHandle;
        if (!engineWindow) {
            return;
        }
        ...
        [engineWindow setFrame:screenFrame display:YES];
        
        // Make the window non-interactive for desktop integration
        [engineWindow setIgnoresMouseEvents:NO]; // We still want mouse events for interaction
        [engineWindow setAcceptsMouseMovedEvents:YES];
    ...
    bool IsDesktopLocked() {
        // TODO: Implement desktop locking detection for macOS
        return false;
    }
